...
阶段 3 编辑器 codec 测试使用。契约正文见 `docs/SCRIPT_EDITOR_CONTRACT.md`，
...
| `server/tests/fixtures/script_v2/<ID>.golden.json` | `json/<ID>.golden.json` | 合法样例期望（拟议前端 Model 字段名） |
| `server/tests/fixtures/script_v2/<ID>.expected.json` | `json/<ID>.expected.json` | 非法样例期望（code + step_path + field） |
| `server/tests/fixtures/script_v2/README.md`、`*.golden/expected.json` 之外的文件 | 不复制 | 服务端测试支持模块（`server/tests/script_v2_contract/`）只存在于服务端 |
...
- **服务端**（saphyr-parser 事件层）是权威解析与非法拒绝方；
