...
.hit-label {
  position: absolute; top: -22px; left: 0; background: var(--accent); color: #202015;
...
.select-box {
  ...
  background: color-mix(in srgb, var(--accent) 12%, transparent); pointer-events: none; z-index: 5;
}
...
.alt-label {
  position: absolute; top: -20px; left: 0; font-size: 12px;
...
.bridge-overlay { position:absolute; border:2px solid var(--accent-2); background:color-mix(in srgb, var(--accent) 12%, transparent); box-shadow:0 0 10px color-mix(in srgb, var(--accent) 25%, transparent); }
...
.loupe {
  ...
  border: 1px solid color-mix(in srgb, var(--accent) 50%, transparent); border-radius: var(--radius-sm); overflow: hidden;
  background: #000; box-shadow: 0 8px 30px rgba(0,0,0,.6);
  pointer-events: none;
}
.loupe canvas { width: 100%; height: 100%; display: block; }
.loupe-tag {
  position: absolute; right: 6px; bottom: 4px; font-size: 12px;
  color: #fff; background: rgba(0,0,0,.55); padding: 1px 5px; border-radius: var(--radius-sm);
  font-family: var(--mono);
}
