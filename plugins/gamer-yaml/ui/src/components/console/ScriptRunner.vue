  ...
  <section class="runner-workspace">
    <div class="resource-toolbar">
      ...
      <details ref="moreEl" class="resource-more"><summary class="btn btn-icon menu-summary" title="更多操作" aria-label="更多操作"><UiIcon name="more" /></summary><div class="resource-menu action-menu">
        <button class="action-menu-item" @click="newTarget" :disabled="ctx.store.running">新建{{ isFunction ? '函数' : '脚本' }}</button>
        <button class="action-menu-item" @click="openRename" :disabled="!editing || ctx.store.running">重命名</button>
        <button class="action-menu-item" @click="openRaw" :disabled="!hasTarget || ctx.store.running">原文编辑</button>
        <button class="action-menu-item" @click="ctx.showYaml = !ctx.showYaml; closeMore()" :disabled="!editing">查看生成 YAML</button>
        <button class="action-menu-item" @click="showRunDetails = !showRunDetails; closeMore()">运行详情</button>
        <button class="action-menu-item" @click="ctx.shell.undo(); closeMore()" :disabled="!editing || !ctx.shell.canUndo || ctx.store.running">撤销</button>
        <button class="action-menu-item" @click="ctx.shell.redo(); closeMore()" :disabled="!editing || !ctx.shell.canRedo || ctx.store.running">重做</button>
        <span class="action-menu-separator" role="separator"></span>
        <button class="action-menu-item danger" @click="removeTarget" :disabled="!hasTarget || ctx.store.running">删除</button>
      </div></details>
    </div>
