...
#[command(
    name = "gamer-launcher",
    version,
    about = "Gamer 便携安装启动器/升级器（单实例锁 / 原子 state / manifest 校验 / 依赖修复 / server 监管）"
)]
pub struct Cli {
    /// 安装根目录；缺省取本 exe 所在目录
...
pub enum Command {
    ...
    /// 查看当前安装状态（只读，不获取单实例锁）
    Status,
    /// 自检：不带 --manifest 检查安装库存；带 --manifest 对任意 release manifest 做完整结构校验
    Doctor {
        /// 对指定 release manifest 文件跑完整校验（fail closed）
