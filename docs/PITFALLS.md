...
- 桌面启动发现与 IPC/CLI 的网页更新检查若各自设置来源，会出现 beta 启动可发现但网页仍访问 stable 的分歧；所有入口统一配置官方发现源，并在每次检查时解析当前通道。

- 单 EXE 首装不能只写 `port`：服务端 TOML 有多个必填字段。启动器与完整包共享 `release/config.default.toml`，服务端测试直接反序列化该模板；真实包验收另用 `-FreshConfig` 移出完整包配置，验证实际 EXE 自行生成配置并启动服务，避免完整包模板掩盖首装缺陷。

- 网页 IPC 更新会主动退出旧服务；CLI 启动器若只等待最初的 Child，会一并销毁 IPC/Job。托管循环须等待更新事务结束并接管新服务或回滚服务，候选继续使用同一管道和会话令牌。

- SQLite 即使只读查询也可能改写已有 `-shm`，对封存快照直接 `inspect` 会使后续回滚 SHA256 失败；诊断前仅复制数据库及其 WAL/SHM 到临时目录，诊断结束清理副本，封存备份始终不被打开。

- 官方插件标签按插件分别递增，不能跨插件比较版本号挑选目录；每次发布携带完整 registry，按通道筛选后取最近公开目录，逐插件再按 SemVer 比较（`beta.10 > beta.2`）。网页从宿主同源下载并校验 SHA256，避免浏览器直接取 GitHub 归档的 CORS 限制。
...
- `tap` / `swipe` 每次经 WASM `__fn` 新建宿主，不能依赖前一步 `find` 缓存的画面尺寸；固定 1000×1000 会把实际 (350,871) 点成 (182,806)。输入转换前读取当前 scrcpy 会话尺寸（仅元数据、不截图），未知尺寸拒绝输入，匹配使用截图自身尺寸；回归必须覆盖非正方形画面和跨调用链。

- 插件 `host/` 新增宿主调用后，独立仓 CI 若仍使用旧 `sdk/lock.json.commit` 会因宿主接口缺失而失败；先推送包含接口的宿主提交，再固定该提交作为集成基线，SDK 字节未变时保留原哈希。

- 插件 ZIP 写入构建时间会让未改版插件的归档 SHA256 改变；发布合集必须按复用锁核对旧包哈希并逐条目确认内容相同后复用旧字节，内容变化则升级插件版本。

- 更新插件发行目录后，干净 checkout 的市场集成测试可能缺少被忽略的 `.gplugin` 产物；CI 和本地完整门禁先通过 `fetch-plugins.ps1 -TestFixturesOnly` 获取发行锁指定的基线并核对 SHA256，发行打包仍严格检查插件源码提交绑定。

- 本地 `main` 停在插件分仓前时直接切换，会因现有 submodule 文件被旧版普通目录覆盖而遭 Git 拒绝；先确认本地主干是远端主干祖先且未被其他 worktree 使用，再快进本地主干引用后切换，不强制覆盖插件目录。

- Vue 的布尔属性 disabled 绑定空字符串仍会禁用按钮：忙碌状态若用字符串描述阶段，模板应绑定 `!!busy`，并通过实际点击测试覆盖空闲态。
- GitHub Release 刚公开后，匿名 latest/资产发现可能暂未同步：发布结果与下载闭环分开复核，保留远端发布记录后稍候刷新，不因发现暂空重复创建 Release。

- Windows 旧工作区的 CRLF 会改变 Vue 生产 scope hash，Rust 本地工具链也可能产生不同 WASM 字节；已发布同版本归档复用必须对实际 CI 工件逐条目核对，不能因本地构建差异放宽检查。
- 多轮 Rust debug 构建会保留多套依赖和增量缓存并耗尽磁盘；先统计 target/debug 占用，清理获准的可重建缓存，批量验收可设置 CARGO_INCREMENTAL=0，删除若被平台拒绝不得改包装绕过。
