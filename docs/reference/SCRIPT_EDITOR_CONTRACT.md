...
> 均不复存在，本文描述的契约随之失效，仅作历史档案保留。**YAML v3 也已在
> Gamer V1 简化计划（2026-09）中整体删除**：现行 V1 语法契约见
> `docs/reference/YAML.md`；前端编辑器契约以 `web/src/script-editor/` 的
> V1 model/codec 实现为准。本文出现的资源布局 / 资源 id 形态与 API 路径
