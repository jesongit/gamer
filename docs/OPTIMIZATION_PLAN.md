...
> 状态：阶段0/1 已完成（2026-08-27），阶段2 进行中  
> 编制日期：2026-08-27  
...
- [x] 增加 `.dockerignore`，至少排除：
  ...
  - [ ] `server/web-dist/`（注：vite 宿主构建产物输出于此，现版 `.dockerignore` 未单列——镜像各 COPY 均不消费该目录、实测上下文仅 1.07MB 无实际瓶颈，归入阶段 7 发布收口统一补充）
  - [x] `web/node_modules/`
  - [x] `.git/`
...
- [ ] 明确敏感字段不得写入日志：密码、Cookie、完整 Authorization、导入文件内容。（注：password 已明确绝不落日志并纳入启动非敏感摘要白名单（4cca76b）；Cookie/Authorization 条目属阶段 2 会话机制的伴随约束，归入阶段 2 验收）
...
- [ ] ffmpeg/adb 可执行性放入 readiness 检查；是否阻止启动由部署模式决定。（注：probe_external_tools 启动探测+warn 不阻断已落地（4cca76b），完整 /health/ready readiness 端点归阶段 4 OBS-001）
...
- [ ] 静态资源、登录和 `/health/live` 可匿名访问。
- [ ] 其他 `/api/**` 全部经过会话鉴权。
- [ ] `/ws/device/:id` 在升级前完成鉴权。
- [ ] `/api/shutdown`、设备控制、脚本运行/停止、模板删除、ZIP 导入列为高风险接口，增加单独测试。
- [ ] 未认证统一返回 401；已认证但被策略禁止返回 403。
...
- [ ] 生产环境默认同源，不额外开放 CORS。
- [ ] 开发模式由 Vite proxy 保持同源体验。
- [ ] 对状态变更请求验证 `Origin`/`Host`。
- [ ] Cookie 使用 `SameSite=Strict`；若以后必须跨站部署，再单独设计 CSRF token。
- [ ] WebSocket 验证 Origin。

#### SEC-004：输入和资源限额

- [ ] 为不同路由设置不同 body limit，而不是所有接口统一 20 MiB。
- [ ] ZIP 导入初始建议限制：
  - [ ] 压缩包 ≤ 20 MiB。
  - [ ] 解压后总量 ≤ 100 MiB。
  - [ ] 文件数 ≤ 500。
  - [ ] 单 YAML ≤ 1 MiB。
  - [ ] 单模板 ≤ 10 MiB。
- [ ] 使用 ZIP entry 声明大小做预检查，并在实际读取时再次计数，防止声明不可信。
- [ ] 图片解码前检查字节数，解码后检查像素总量，防止超大尺寸图片消耗内存。
- [ ] 日志查询 `limit` 限制在合理范围，例如 1～1000。
- [ ] 控制命令校验坐标、时长、文本长度和应用包名，不再用大量 `unwrap_or(0)` 静默接受缺失字段。

### 8.3 验收矩阵

- [ ] 未登录访问设备列表返回 401。
- [ ] 未登录调用 shutdown 返回 401，服务保持运行。
- [ ] 未登录建立 WebSocket 失败。
- [ ] 登录后 REST、WebSocket、DataChannel 正常工作。
- [ ] 登出后旧 Cookie 立即失效。
- [ ] 跨 Origin 状态变更请求被拒绝。
- [ ] 超限 ZIP、ZIP slip、重复文件、超大图片均返回 4xx，进程内存不持续增长。
- [ ] 浏览器刷新后会话行为符合设计，不依赖 localStorage 伪 token。
...
- [ ] 统一接管手动脚本和 Scheduler 的运行状态。
- [ ] 使用 RAII guard 保证所有退出路径都执行 `devices.run_end` 并释放设备占用。
- [ ] panic、任务取消、连接失败、引擎报错和正常完成都必须进入终态。
- [ ] 服务关闭时先停止接收新任务，再取消/等待活动任务。
- [ ] 停止动作按 `run_id` 定位，不按脚本名猜测。

#### RUN-002：调整 API

- [ ] 启动脚本立即返回 `202 Accepted` 和 `run_id`。
- [ ] 立即运行定时任务同样立即返回，不占用 HTTP 连接。
- [ ] 增加统一运行查询，例如 `GET /api/runs/:run_id`。
- [ ] 增加设备当前运行查询，例如 `GET /api/devices/:id/run` 返回完整 RunRecord 摘要。
- [ ] 增加按 `run_id` 停止接口。
- [ ] 旧的 script status/stop 接口保留一个迁移版本，内部转到 RunManager，并在前端迁移后删除。

#### RUN-003：前端状态迁移

- [ ] store 以 `runId` 为主键，不再以 `scriptId` 充当执行实例 ID。
- [ ] 页面刷新后按设备恢复当前 run。
- [ ] 冲突时展示正在运行的脚本、来源和开始时间。
- [ ] 定时任务立即运行按钮收到 202 后立即恢复，不等待脚本结束。
- [ ] 停止按钮只停止当前 run。

#### RUN-004：调度幂等

- [ ] 新增运行或触发表，持久化 `(task_id, scheduled_at)`。
- [ ] 建立唯一约束，保证同一计划触发点至多创建一次运行。
- [ ] 明确 misfire 策略：
  - 推荐默认：服务恢复后只补最近一次、且延迟不超过配置窗口的触发。
  - 过期更久的触发记录为 skipped，不批量补跑。
- [ ] `last_run_at` 使用可排序的 UTC 时间格式或整数时间戳，不再依赖本地格式字符串。
- [ ] 系统时区/DST 变化需要测试。
...
- [ ] 同一设备手动运行两个不同脚本，第二个返回 409。
- [ ] 手动脚本运行时定时任务命中，定时任务按策略记录冲突/跳过，不注入控制。
- [ ] 两个定时任务同秒命中同一设备，只有一个取得执行权。
- [ ] 两台设备可以并行运行。
- [ ] 启动阶段连接失败后设备锁被释放。
- [ ] 运行中请求停止，最终状态为 cancelled，run count 归零。
- [ ] 服务重启不会重复执行已经持久化的 scheduled_at。
- [ ] task-now API 在任务完成前已经返回 202。
...
- [ ] 新建文件写入工具：同目录临时文件 → 写入 → flush/sync → rename/replace。
- [ ] 脚本保存、模板上传和配置生成使用统一工具。
- [ ] Windows 下验证替换已有文件的行为，避免 rename 语义差异。
- [ ] 写入失败时旧文件保持完整。

#### DATA-002：分区导入事务化

- [ ] `confirm=false` 只做解析和冲突报告。
- [ ] `confirm=true` 先解压到同文件系统 staging 目录。
- [ ] 完成文件类型、数量、大小和 YAML 编码校验后再提交。
- [ ] 覆盖文件前建立可恢复备份或以目录交换实现提交。
- [ ] 任一文件失败时不留下半导入状态。
- [ ] 导入完成后清理 staging；服务启动时清理过期 staging。

### 10.2 SQLite

#### DATA-003：数据库写入治理

- [ ] 配置 `busy_timeout`、WAL 和必要的 foreign key。
- [ ] 不在 Tokio 核心线程直接执行可能较慢的 rusqlite 操作。
- [ ] 方案优先级：
  1. 单独 DB worker/actor，异步通道提交操作。
  2. 低频查询使用 `spawn_blocking`。
  3. 不建议只把当前 Mutex 换成异步 Mutex，因为同步 SQLite 调用仍会阻塞 executor。
- [ ] 日志写入按小批量事务提交，例如 100 条或 250ms 一批；异常退出允许损失极少量 debug 日志，但 success/error 终态必须可靠落盘。
...
- [ ] `/health/live`：进程事件循环可响应即可。
- [ ] `/health/ready`：检查数据目录、SQLite、scrcpy jar，并报告 adb/ffmpeg 探测结果。
- [ ] readiness 返回结构化 JSON，不泄露密码和主机敏感路径。
- [ ] Docker healthcheck 使用 readiness 或轻量专用端点。
...
- [ ] 准备固定 H.264 GOP、截图和模板 fixture，不使用实时设备作为唯一基准。
- [ ] 分别记录：
...
- [ ] 缓存内容包括：灰度图、尺寸、缩放版本、均值、方差和 NCC 所需数组。
...
- [ ] `FrameCache` 暴露单调递增的帧序号和最近帧到达时间。
- [ ] 同一设备、同一帧序号上的并发截图请求共享一个 in-flight decode future。
- [ ] 已解码结果只允许在很短新鲜度窗口内复用，初始建议 50～100ms，并以指标调整。
- [ ] 新帧到达后不得长期返回旧 PNG。
- [ ] 分辨率/config 代数变化立即失效缓存。
- [ ] `find` 主模板和 block 是否共享同一截图必须保持现有语义；第一版只合并真正并发或同帧请求，不主动改变“每模板独立取新截图”。
...
- [ ] 跨文件函数解析与文件系统寻址分离。
- [ ] 重构不改变 YAML 语义；如必须改变，另起破坏性提交并同步文档。
...
- [ ] 匿名访问所有受保护 API 均失败。
- [ ] shutdown 和控制接口不能被跨站页面调用。
- [ ] Cookie/Authorization 不进入日志。
- [ ] 大包、ZIP bomb、恶意文件名和超大图片均被限制。
- [ ] gamer 容器在非 USB 场景不使用 privileged。
...
- [ ] README 删除“连接自动启动应用”等过期说明。
- [ ] README 的帧缓存描述改为“GOP 帧环 + 按需 ffmpeg 解码”。
- [ ] README 的 Docker profile 和前端构建步骤与实际一致。
- [ ] 补充鉴权、Cookie、HTTPS 反向代理和管理脚本说明。
- [ ] 补充 RunManager、run_id 和冲突行为。
- [ ] 新的环境/部署坑追加到 `docs/PITFALLS.md`。
- [ ] 若 YAML 语义未变，不改写 `docs/YAML.md`；若有变化则同步引擎、前端校验、模板和文档。
...
- 未完成任务及原因：`server/web-dist` 显式排除项未加（镜像 COPY 不消费该目录、实测上下文 MB 级无瓶颈，顺延阶段 7）；敏感字段日志约束中 Cookie/Authorization 部分依赖会话机制落地，归入阶段 2 验收；完整 /health/ready readiness 端点归 OBS-001（阶段 4），启动期 probe_external_tools 探测已预留；USB 直通真机验证受 Windows Docker Desktop 平台限制，仅完成 compose 配置校验，真机回归归阶段 7 设备矩阵
