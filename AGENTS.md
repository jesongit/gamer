...
- 连接：浏览器 → `POST /api/devices/:id/connect`（scrcpy 会话）→ WS `/ws/device/:id` 信令（offer/answer）→ WebRTC 视频轨 + DataChannel `control`（触控/按键/文本）；脚本运行时引擎的 tap/swipe/匹配命中可视化事件也经该 DataChannel **反向**推送给投屏页面（`{"type":"se","ev":...}`，engine `emit` → viewers 注册表 `control_dc`，定时任务运行同样生效）
