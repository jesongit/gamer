...
- 配置变更（PUT /api/devices/:id）：仅**投屏相关参数**（kind/addr/screen_mode/vd_res/vd_dpi/fps，`session_affecting_change` 按生效值归一比较）变更才踢 viewer + 拆会话（前端 onclose → 自动重连恢复画面）；仅改名称/应用包名不拆会话、投屏不中断；投屏参数变更遇脚本运行中仍不踢不拆（运行守卫），新配置下次连接才生效。前端设备管理收在投屏工具条，两组布局：**设备区**（设备下拉/连接/刷新 + 「更多」下拉：新增/设置/删除）｜**应用区**（「应用」下拉：Android 运行目标唯一配置入口，选中即 PUT 全量保存 pkg 不拆会话/「读取」拉已装应用列表（force 绕 5 分钟缓存）/「启动」/游戏模式与映射下拉 + 「功能」下拉：粘贴/截图/按键/停止应用等；逻辑在 useConsoleDeviceManager），配置编辑走「⚙️ 设置」弹窗（DeviceSettingsModal，显式保存/取消，不再自动保存防抖，弹窗内不编辑 pkg）
