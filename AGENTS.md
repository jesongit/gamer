...
- **业务归扩展**：`extensions/gamer_yaml/`（**YAML v3 唯一方案**：`yaml_vnext.rs` parse+lower+函数库解析、`yaml_extension.rs` WASM 契约+NativeYamlHost+原生参考解释器、`wasm_host.rs` LazyYamlWasmtimeRuntime、`engine/runner_adapter.rs` v3-only EngineExecutor、`timer_yaml.rs` 定时 runner、`task_params.rs` 任务参数门禁；v2 parser/AST/loader/执行器/编辑器已彻底删除，非 `version: 3` 一律 `yaml.v3.version(.missing)` 诊断，无 fallback 无迁移工具）与 `extensions/keymap/`（keymap DSL 与 WASM 运行时适配）。**Extension ≠ WASM**：cron 是 Native Extension；WASM 只是扩展运行时之一（`extensions/wasm.rs`）。
...
- 边界由测试锁死：`server/src/architecture_guard_tests.rs`（七测试：源码边界 53 条白名单双向校验/依赖方向/v3-only 禁符/生命周期全链/裸核/YAML 隔离/Keymap 隔离）+ `web/src/core-shell-boundary.test.js`。
...
| `server/src/extensions/gamer_yaml/` | YAML v3 扩展边界（v2 已删）：`yaml_vnext.rs`（v3 surface：parse+lower+函数库解析，严格诊断）、`yaml_extension.rs`（WASM 契约 + NativeYamlHost + 原生参考解释器 + e2e）、`wasm_host.rs`（LazyYamlWasmtimeRuntime：epoch interruption 取消兜底、nonce 注入、`__event` 私有事件通道）、`engine/runner_adapter.rs`（v3-only EngineExecutor + call 目标命名空间解析）、`engine/run_target.rs`（RunTarget/RunSpec/TypedValue，wire 不变）、`engine/params.rs`（标量助手）、`error.rs`（ScriptError 诊断载体）、`resources.rs`（v3-only 校验：scripts=load、functions=parse_function_library、模板重命名 v3 AST 引用改写）、`task_params.rs`（v3 参数桥 + psig1 签名门禁）、`entrypoint_descriptor.rs`（参数 schema 描述器）、`timer_yaml.rs`（v3-only runner + Registrar） |
...
| `web/src/components/task/` | 通用任务表单支撑：runner-editors.ts（`RunnerEditorContribution` 契约注册表）、builtin-runner-editors.ts（gamer.yaml 执行目标+payload 编辑器，参数表单走 `GET /api/runners/:runner_id/entrypoint?entrypoint=<id>` schema API，前端不解析 YAML 取参）、gamer-yaml-resources.ts（分区候选经通用资源 API） |
...
| `web/src/script-editor/` | YAML v3 可视化编辑器核心（model=唯一编辑源（19 类步骤判别联合）、codec=YAML↔Model、校验/诊断、命令栈撤销重做、components 步骤画布/卡片/参数表单（DefaultsEditor 承载 defaults）；call 卡片目标为分区候选下拉（target 语法 `script:<资源id>` / `function:<文件短路径>/<函数名>`，targets.ts 注入契约），选定即按目标声明自动生成 args（默认值预填），新插入步骤自动展开）；契约见 docs/reference/SCRIPT_EDITOR_CONTRACT.md |
