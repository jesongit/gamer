...
- **退路（计划 §15 已预置）**：若该结论被推翻或法务要求收紧 → 公开发行退回 **lite 包 / 内部 seed 路线**（full 包不公开，仅内部使用；用户自装 platform-tools，Gamer 以 `system/custom` 模式引用）。ARC-005 未闭环前**禁止公开发布 full 包**。
