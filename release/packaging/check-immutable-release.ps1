...
# draft/published Release；Registry 模式在 push 前检查 GHCR version tag。任何已存在的
# version tag 都必须显式带入相同 digest 才能继续，默认路径 fail closed，绝不覆盖。
...
function Assert-Tag {
    ...
    if ($Value -notmatch '^v[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$') {
        Fail "tag 不合法: $Value（须为 v<semver>）"
    }
}
...
function Get-GitTagCommit {
    param([string]$Value)
    $rows = @(& git ls-remote origin ("refs/tags/{0}^{{}}" -f $Value) 2>&1)
    $code = $LASTEXITCODE
...
if ($inspectCode -eq 0) {
    $match = [regex]::Match($inspect, '(?im)^\s*Digest:\s*(sha256:[0-9a-f]{64})\s*$')
