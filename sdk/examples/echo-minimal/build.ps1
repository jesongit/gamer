# Build script for the "echo-minimal" example plugin (Windows PowerShell).
# Produces dist/com.example.echo-<version>.gplugin using the repo's own
...
if ($Signer -eq "") {
    # sdk/examples/echo-minimal -> repo root is three levels up.
...
if ($LASTEXITCODE -ne 0) { exit 1 }
$module = Join-Path $targetDir "wasm32-unknown-unknown\release\echo_minimal_plugin_guest.wasm"
